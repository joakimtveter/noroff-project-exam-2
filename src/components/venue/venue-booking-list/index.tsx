...
export default function VenueBookingList(props: VenueBookingListProps) {
...
function BookingItems(props: Booking) {
    ...
    <ListItemAvatar>
        <Stack component='p' alignItems='center'>
            <Box component='span' sx={{fontSize: '20px', margin: '0 0 -3px', color: 'red'}}>{dateToDayNumber(fromDate)}</Box>
        <Box component='span' sx={{fontSize: '12px', margin: 0}}>{dateToShortMonth(fromDate).toUpperCase()}</Box>
        </Stack>
    </ListItemAvatar>
    <ListItemText
        primary={`${props.guests} ${props.guests > 1 ? 'Guests' : 'Guest'} - ${stayDuration} ${stayDuration > 1 ? 'Nights' : 'Night'}`}
        secondary={`${from} - ${to}`}
        />
    </ListItem>
    )
