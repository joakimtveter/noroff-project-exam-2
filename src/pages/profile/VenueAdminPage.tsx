...
export default function VenueAdminPage() {
    ...
    if (!venueId) navigate('/404');
    const { data, error, isLoading } = useGetVenueByIdQuery(venueId || '');

  return (
      <Layout>
        {error ? ( <p>Oh no, there was an error</p> ) : isLoading ? ( <CircularProgress /> ) : data ? (
        <Box>
        <Typography component='h1' variant='h2'>Venue Admin {venueId} </Typography>
        <Typography component='h2' variant="h4">Bookings</Typography>
